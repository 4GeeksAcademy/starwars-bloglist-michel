const getState = ({ getStore, getActions, setStore }) => {
	return {
		...
		actions: {
			...
			fetchPeople: async () => {
				const res = await fetch('https://swapi.tech/api/people/').then(response => response.json());
				const charactersWithDetails = await Promise.all(res.results.map(async character => {
					const detailedResponse = await fetch(character.url).then(response => response.json());
					return { ...character, ...detailedResponse.result.properties };
				}));
				setStore({ characters: charactersWithDetails });
			},

		fetchPlanets: async () => {
				const res = await fetch('https://swapi.tech/api/planets/').then(response => response.json());
				const planetsWithDetails = await Promise.all(res.results.map(async planet => {
					const detailedResponse = await fetch(planet.url).then(response => response.json());
					return { ...planet, ...detailedResponse.result.properties };
				}));
				setStore({ planets: planetsWithDetails });
			},
			fetchShips: async () => {
				const res = await fetch('https://swapi.tech/api/starships/').then(response => response.json());
				const shipsWithDetails = await Promise.all(res.results.map(async ship => {
					const detailedResponse = await fetch(ship.url).then(response => response.json());
					return { ...ship, ...detailedResponse.result.properties };
				}));
